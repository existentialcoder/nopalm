...
const packageOperation = async (pkg, packageManager, operation) => {
  const op = operation === 'install' ? (packageManager === 'npm' ? 'i' : 'add') : 
...
const getPackageManagers = async (dir) => {
  return Promise.all(constants.packageManagers.map(async pkgManager => {
    try {
